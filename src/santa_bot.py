...
def restrict_to_group(message: str):
...
class SantaBot:
    ...
    @restrict_to_group("Use this command in a group to start new Secret Santa game")
    ...
    @require_me
    @restrict_to_group("Use this command in a group to shuffle a Secret Santa game")
    async def _handle_shuffle(self, update: Update, _: CallbackContext):
        ...
        group: ChatFullInfo = await self.__application.bot.get_chat(game.group_id)

        async def update_user(santa_id, recipient_id):
            recipient: ChatFullInfo = await self.__application.bot.get_chat(recipient_id)
            ...
            await self.__application.bot.send_message(
                ...
                " ".join([f"You have been assigned as [{recipient_name}](tg://user?id={recipient_id})'s",
                          ...
                          f"Secret Santa for '__{game.name}__' in *{group.title}*!"]),
